        ...
        <div className="relative overflow-hidden rounded-xl md:rounded-2xl bg-white dark:bg-card-dark p-2.5 md:p-6 shadow-md border border-gray-100 dark:border-border-dark group h-full flex flex-col justify-between">
          ...
          <div className="relative z-10 flex flex-col h-full justify-between gap-1.5 md:gap-0">
            <div className="flex items-center justify-between md:mb-4">
              ...
              <div
                className={`flex items-center text-[8px] md:text-xs font-medium px-1.5 md:px-2.5 py-0.5 md:py-1 rounded-full ${data.balanceChangePct && data.balanceChangePct >= 0 ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-rose-50 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400'}`}
              >
                <TrendingUp
                  className={`h-2 w-2 md:h-3 md:w-3 mr-0.5 md:mr-1 ${data.balanceChangePct && data.balanceChangePct < 0 ? 'rotate-180' : ''}`}
                />
            ...
            <div>
              <p className="text-gray-500 dark:text-gray-400 text-[9px] md:text-sm font-medium mb-0.5 md:mb-1 leading-tight">
                Saldo Total
              </p>
        ...
        <div className="relative overflow-hidden rounded-xl md:rounded-2xl bg-white dark:bg-card-dark p-2.5 md:p-6 shadow-md border border-gray-100 dark:border-border-dark group h-full flex flex-col justify-between">
          ...
          <div className="relative z-10 flex flex-col h-full justify-between gap-1.5 md:gap-0">
            <div className="flex items-center justify-between md:mb-4">
              ...
              <div
                className={`flex items-center text-[8px] md:text-xs font-medium px-1.5 md:px-2.5 py-0.5 md:py-1 rounded-full ${data.incomeChangePct && data.incomeChangePct >= 0 ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400'}`}
              >
                {data.incomeChangePct
                  ? `${data.incomeChangePct > 0 ? '+' : ''}${data.incomeChangePct.toFixed(1)}%`
                  : '-'}
              </div>
            </div>
        ...
        <div className="relative overflow-hidden rounded-xl md:rounded-2xl bg-white dark:bg-card-dark p-2.5 md:p-6 shadow-md border border-gray-100 dark:border-border-dark group h-full flex flex-col justify-between">
          ...
          <div className="relative z-10 flex flex-col h-full justify-between gap-1.5 md:gap-0">
            <div className="flex items-center justify-between md:mb-4">
              ...
              <div
                className={`flex items-center text-[8px] md:text-xs font-medium px-1.5 md:px-2.5 py-0.5 md:py-1 rounded-full ${data.expensesChangePct && data.expensesChangePct > 0 ? 'bg-rose-50 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400' : 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400'}`}
              >
                {data.expensesChangePct
                  ? `${data.expensesChangePct > 0 ? '+' : ''}${data.expensesChangePct.toFixed(1)}%`
                  : '-'}
              </div>
            </div>
