  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700">
        <p className="text-sm font-medium text-gray-900 dark:text-white mb-1">{data.category}</p>
        <div className="flex items-center justify-between gap-4">
          <span className="text-sm text-gray-500 dark:text-gray-400">Gasto:</span>
          <span className="text-sm font-semibold text-red-500">{formatCurrency(data.amount)}</span>
        </div>
