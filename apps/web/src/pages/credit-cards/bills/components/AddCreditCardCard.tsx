...
interface AddCreditCardCardProps {
  readonly onClick: () => void;
}

export function AddCreditCardCard({ onClick }: Readonly<AddCreditCardCardProps>) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex w-[280px] min-w-[280px] h-[172px] flex-shrink-0 cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-border dark:border-border-dark bg-card/50 dark:bg-card-dark/50 transition-all duration-200 hover:opacity-100 hover:border-primary dark:hover:border-primary-dark hover:bg-card dark:hover:bg-card-dark focus:outline-none opacity-80"
