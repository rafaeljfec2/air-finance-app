  ...
  return (
    <div className="relative w-full max-w-lg mx-auto md:max-w-none">
      <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden text-left">
        <div className="px-5 py-4 md:px-6 md:py-5 border-b border-gray-100 bg-gradient-to-br from-gray-50 to-white">
          ...
          <div className="space-y-1.5 text-sm text-gray-600 leading-relaxed">
            ...
            <p className="text-gray-400">Os demais pilares sustentam essa capacidade.</p>
          </div>
        </div>
            ...
            {PREVIEW_PILLARS.map((pillar, index) => (
              <li key={pillar.name} className="relative flex gap-3 pb-4 last:pb-0">
                <div className="relative flex w-3 shrink-0 flex-col items-center">
                  <span className="z-10 mt-1.5 h-2.5 w-2.5 rounded-full border-2 border-emerald-500 bg-white" />
                  {index < PREVIEW_PILLARS.length - 1 ? (
                    <span className="absolute top-4 bottom-0 w-px bg-gray-200" aria-hidden />
                  ) : null}
                </div>
                <div className="min-w-0 flex-1 rounded-xl border border-gray-100 bg-gray-50/80 px-3 py-2.5">
                  ...
                  <p className="text-base font-bold tabular-nums text-gray-900">{pillar.value}</p>
                  <p className="text-xs text-gray-500 mt-0.5">{pillar.question}</p>
                  <p className="text-[11px] font-medium text-emerald-600 mt-1.5">Explorar</p>
                </div>
              </li>
            ))}
          </ol>
